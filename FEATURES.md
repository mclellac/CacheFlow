...
### 8. Request Replay with Modification
**Problem:** Testing "what if" scenarios requires switching to a different tool (like curl).
**Feature:** Right-click a node to "Replay Request", allowing the user to modify headers or method (e.g., change `GET` to `HEAD`, add `Accept-Encoding: gzip`) and see the new result graph.
