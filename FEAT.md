...
*   **Renamed Concepts:**
    ...
    *   `Host Overrides` -> **"Host Header Exceptions"**.
*   **Smart Presets (The "I use Cloudflare" Button):**
    *   When adding a layer, offer a **"Load Preset"** button.
    *   Presets: "Cloudflare (Standard)", "AWS ALB", "Nginx (Reverse Proxy)", "Varnish (Default)".
    *   Loading a preset pre-fills standard headers (like `CF-Cache-Status` or `X-Cache`) and behaviors.
