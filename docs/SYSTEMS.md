...
*   **Path Matching**: Define rules to match specific request paths (e.g., `/api/v1`).
*   **Backend Selection**: Route requests to specific backend applications based on the matched path.
*   **Path Rewriting**: Automatically modify the request path (e.g., using regex substitution) before forwarding it to the backend. This simulates `regsub` behavior common in Varnish and Nginx.
