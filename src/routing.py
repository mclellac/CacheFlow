...
class RouteCalculator:
    ...
    def _apply_rule(rule: Dict[str, Any],
                    target_path: str) -> Optional[Tuple[str, str, Optional[str]]]:
        ...
        if backend_host:
            next_base = (f"https://{backend_host}"
                         if not backend_host.startswith('http') else backend_host)
    ...
    def calculate_next_hop(layer_config: Dict[str, Any],
        ...
        if default_host:
            ...
            next_base = (f"https://{default_host}"
                         if not default_host.startswith('http') else default_host)
