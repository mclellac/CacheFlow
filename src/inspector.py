...
class HeaderInspector:
    def __init__(self, config):
        ...
        """
        print(f"[DEBUG] HeaderInspector.__init__: Initializing with config: {config}")
        self.config = config
        self.dns_servers = []
    ...
    def resolve_host(self, hostname):
        if not self.dns_servers:
            print(f"[DEBUG] HeaderInspector.resolve_host: No custom DNS servers. Returning original hostname '{hostname}'.")
        ...
        try:
            ...
            if answers:
                ip = str(answers[0])
                print(f"[DEBUG] HeaderInspector.resolve_host: Resolved '{hostname}' to '{ip}' using servers {self.dns_servers}.")
                return ip
        except Exception as e:
            # Fallback or raise? Let's return hostname to let requests fail naturally or use system
            print(f"[DEBUG] HeaderInspector.resolve_host: Custom DNS resolution failed for {hostname}: {e}")
        return hostname

    def run_inspection(self, test_path=None):
        print("[DEBUG] HeaderInspector.run_inspection: Starting inspection run.")
        if test_path is None:
            test_path = self.config.get('test_path', '/')

        if not test_path.startswith('/'):
            test_path = '/' + test_path
        print(f"[DEBUG] HeaderInspector.run_inspection: Using test path: '{test_path}'")
        ...
        for i, layer in enumerate(layers_to_inspect):
            print(f"\n[DEBUG] HeaderInspector.run_inspection: --- Processing layer: {layer.get('name')} ---")
            ...
            if path_match_patterns:
                print(f"[DEBUG] HeaderInspector.run_inspection: Checking path against patterns: {path_match_patterns}")
                matched = False
                for pattern in path_match_patterns:
                    if fnmatch.fnmatch(test_path, pattern):
                        print(f"[DEBUG] HeaderInspector.run_inspection: Path '{test_path}' matched pattern '{pattern}'.")
                        matched = True
                        break
                if not matched:
                    print(f"[DEBUG] HeaderInspector.run_inspection: Path '{test_path}' did not match any patterns. Skipping layer.")
                    continue
            ...
            if 'host_overrides' in layer:
                for override in layer['host_overrides']:
                    if fnmatch.fnmatch(test_path, override['path_pattern']):
                        ...
                        break
            print(f"[DEBUG] HeaderInspector.run_inspection: Host header override is: '{host_header_override}'")

            # Prepare Request
            base_url = layer['host_url'].rstrip('/')
            parsed_url = urlparse(base_url)
            print(f"[DEBUG] HeaderInspector.run_inspection: Parsed base URL: {parsed_url}")
            ...
            print(f"[DEBUG] HeaderInspector.run_inspection: Final Host header will be: '{final_host_header}'")

            # Resolve DNS if needed
            target_ip = hostname
            if self.dns_servers:
                target_ip = self.resolve_host(hostname)
            print(f"[DEBUG] HeaderInspector.run_inspection: Target IP for connection is: '{target_ip}'")
            ...
            print(f"[DEBUG] HeaderInspector.run_inspection: Final request URL: '{url}'")
            ...
            print(f"[DEBUG] HeaderInspector.run_inspection: Final request headers: {headers}")

            # Make Request
            try:
                print(f"[DEBUG] HeaderInspector.run_inspection: Making GET request to {url}...")
                ...
                print(f"[DEBUG] HeaderInspector.run_inspection: Request successful. Status: {response.status_code}")
            except Exception as e:
                layer_result = {
                    'name': layer['name'],
                    'description': layer.get('description', ''),
                ...
                print(f"[DEBUG] HeaderInspector.run_inspection: Request FAILED. Error: {e}")
