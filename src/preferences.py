...
class PreferencesWindow(Adw.PreferencesWindow):
    ...
    def do_export_config(self, _row):
        ...
        # But the request is to export everything including top level domain.
    ...
    def on_config_imported(self, data):
        ...
        if isinstance(data, list):
            ...
            if data and "layers" in data[0]:
                # It's a list of configs (each item has 'layers' key)
                log.info("Importing list of configurations.")
                self._import_full_configs(data)
            else:
            ...
            if "layers" in data and "name" in data:
                log.info("Importing single configuration object.")
                self._import_full_configs([data])
            else:
    ...
    def _import_full_configs(self, configs):
        ...
        # For each config, check if ID exists, update if so, else add.
        # Or should we wipe and replace? Merging is safer.
        ...
        for new_conf in configs:
            ...
            if conf_id and conf_id in existing_ids:
                # Update existing
                self.config_manager.save_configuration(conf_id, new_conf)
            else:
                # Add new
                # If ID is missing, one will be generated by add_configuration
                # But add_configuration takes arguments.
                # Use internal add logic or exposed method.
                # add_configuration generates a new ID.
                # We should try to preserve ID if provided to support syncing?
                # The ConfigManager.add_configuration generates a new ID.
                # Let's verify if ConfigManager has a method to add raw dict.
                # It doesn't explicitly. We can use _save_configs with the full list.
                # But that requires updating the whole list.

                # Let's rely on add_configuration for new ones to ensure safety
                self.config_manager.add_configuration(
                    new_conf.get("name"),
                    new_conf.get("entry_point", new_conf.get("name")), # entry_point fallback
                    layers=new_conf.get("layers", [])
                )
            count += 1

        self.refresh_config_list()
        self.add_toast(Adw.Toast.new(f"Imported {count} configurations."))
